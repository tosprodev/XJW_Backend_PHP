...
if ($result->num_rows > 0) {
 ...
 break;
			...
			/*----------------------------------------------------------- Delete Block dates ----------------------------------------------------*/
				...
				$start = 0; 
				$limit = 15; 
				$total = mysqli_num_rows(mysqli_query($conn, "SELECT id FROM msgs WHERE cid = '$cid' ORDER BY id DESC"));
				$page_limit = ceil ($total/$limit); 
				if($page<=$page_limit){ 
				$start = ($page - 1) * $limit; 
				$sql = "SELECT * from msgs WHERE cid = '$cid' ORDER BY id DESC limit $start, $limit";
				$result = mysqli_query($conn,$sql); 
				$res = array(); 
				while($row = mysqli_fetch_array($result)){
				array_push($res, array(
					...
					if (isset($_POST['uid'])) {
						...
						$query = mysqli_query($conn, "SELECT id FROM `chat` WHERE uid='".$uid."' AND pid='".$pid."'");
							if(mysqli_num_rows($query) > 0){
									...
									$Sql_Query = "insert into chat (uid,pid,btype,bid) values ('$uid','$pid','$btype','$bid')";
											if(mysqli_query($conn,$Sql_Query)){
