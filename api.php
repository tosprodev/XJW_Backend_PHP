...
if ($result->num_rows > 0) {
 ...
 break;
			...
			/*----------------------------------------------------------- Delete Block dates ----------------------------------------------------*/
				...
				if($page<=$page_limit){ 
				$start = ($page - 1) * $limit; 
				$sql = "SELECT chat.id, chat.uid, chat.pid, chat.btype, chat.bid, users.*, practitioner.* FROM chat INNER JOIN users ON chat.uid = users.id INNER JOIN practitioner ON chat.pid = practitioner.id WHERE chat.uid = $uid limit $start, $limit";
				$result = mysqli_query($conn,$sql); 
				$res = array(); 
				while($row = mysqli_fetch_array($result)){
				array_push($res, array(
				...
				case 'add_message_ua': 
					...
					$sheet = $spreadsheet->getActiveSheet(); 
							...
							$tempc['invoice_id'] = "Service Charge : $ ".round($total_fee);
							$tempc['uid'] = "Transaction Charge : $ ".round($total_t_fee);  
							$tempc['cur_time'] = "Grand Total : $ ".round($get_total);
